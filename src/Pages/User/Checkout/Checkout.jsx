...
export default function Checkout() {
  ...
  const { product_id, cart_quantity, productName, productPrice, productImage } = location.state || {}
  console.log(product_id, cart_quantity, productName, productPrice, productImage)
  ...
  const handleBuyNow = async () => {
    console.log(value, valueDelivery, valueAddress)
    const buyNow = {
  ...
  useEffect(() => {
    if (data?.data?.data && data.data.data.length > 0) {
      setValue(data.data.data[0].payment_id) // Lấy giá trị mặc định từ phần tử đầu tiên
    }
    if (getDelivery?.data?.data && getDelivery.data.data.length > 0) {
      setvalueDelivery(getDelivery?.data?.data[0].delivery_id)
    }
  ...
  const selectedDelivery = useMemo(() => {
    return getDelivery?.data?.data.find((element) => element.delivery_id == valueDelivery)
  ...
  return (
    <div className='px-24 grid grid-cols-1 md: lg:grid-cols-9 bg-[#e5e5e5] gap-y-3'>
      ...
      <div className='left  lg:col-span-6 bg-white p-5 rounded-sm mt-1'>
        ...
        <div className='grid lg:grid-cols-2 gap-5'>
          {data?.data?.data.map((element) => {
            return (
              <div className='bg-[#e5e5e5] rounded-md p-2 font-semibold mt-4 w-[70%]' key={element.payment_id}>
                <Radio.Group onChange={onChange} value={value} className='flex items-center space-x-5 justify-between '>
                  <Radio value={element.payment_id}>{element.payment_method}</Radio>
          ...
          {selectedDelivery && (
            <div className='flex mt-4 lg:flex-row flex-col'>
              <div className='w-[70%] flex-col'>
                <div className='font-medium'>Vận Chuyển {selectedDelivery.delivery_method}</div>
                <div className='text-sm mt-1'>
                  Dự kiến giao hàng: {selectedDelivery.delivery_description} ngày không bao gồm thứ 7, chủ nhật
                </div>
              </div>
        ...
        <div className='bg-[#e5e5e5] mt-5 mr-5 mb-5 rounded-sm '>
          <div className=' border-2 py-3 px-2 mt-3'>
            ...
            <div className='flex justify-between  py-3 px-2  my-1 '>
              ...
              <div className='text-[#F22121] font-semibold'>{}</div>
            </div>
      ...
      <ModalDelivery isModalDelivery={isModalDelivery} setModalDelivery={setModalDelivery} getDelivery={getDelivery} valueDelivery ={valueDelivery} setvalueDelivery={setvalueDelivery} />
    </div>
  )
}
